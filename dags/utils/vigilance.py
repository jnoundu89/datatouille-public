...
def map_alerts_to_stores(
    ...
    """Map meteorological alerts to GIE / Store locations."""
...
def insert_gie_alerts(cur, gie_alerts: list[dict[str, Any]]) -> None:
    """Insert store warnings into vigilance_gie_alerts PostgreSQL table."""
    if not gie_alerts:
        return
    ...
    execute_values(
        cur,
        """
        INSERT INTO vigilance_gie_alerts (
            gie_name, department_code, department_name, phenomenon_name,
            begin_time, end_time, alert_level, color_hex,
            extracted_at, extraction_date
        ) VALUES %s
        ON CONFLICT (gie_name, phenomenon_name, begin_time, extraction_date)
        DO UPDATE SET
