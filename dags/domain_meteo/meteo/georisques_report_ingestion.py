...
Extracts natural and technological risk data from GeoRisques API and maps them to GIE locations.
...
from utils.validation import CommuneProfileRecord, GieRiskRecord, RiskDetailRecord, validate_records
...
@dag(
    ...
    description="Risk data ingestion from GeoRisques API for GIE locations",
...
def georisques_report_ingestion():
    ...
    def extract_risks() -> dict[str, Any]:
        """Extract risk data from GeoRisques API for all GIE communes (parallel)."""
        cfg = get_meteo_config()
        bucket_name = cfg["georisques_bucket_name"]
        ...
        # Load GIE mapping to get INSEE codes with commune info for fallback
        ...
        insee_to_commune = {}
        for gie_data in gie_mapping.values():
    ...
    def transform_risks(extract_result: dict[str, Any]) -> dict[str, Any]:
        ...
        # Medallion Silver: Validate and upload as staging Parquets
        if commune_profiles:
            valid_profiles = validate_records(
                commune_profiles, CommuneProfileRecord, pipeline_name="georisques_profiles"
            )
            upload_parquet("datatouille", "landing/stg_georisques_commune_profiles.parquet", valid_profiles)

        if risk_details:
            valid_details = validate_records(risk_details, RiskDetailRecord, pipeline_name="georisques_details")
            upload_parquet("datatouille", "landing/stg_georisques_risk_details.parquet", valid_details)
    ...
    def transform_gie(transform_result: dict[str, Any], **context) -> dict[str, Any]:
        """Map commune risks to GIE locations."""
        cfg = get_meteo_config()
        bucket_name = cfg["georisques_bucket_name"]
        ...
        # Map GIEs
        gie_records = []
        for gie_name, gie_data in gie_mapping.items():
        ...
        gie_key = get_partitioned_key(
            domain="meteo", source="georisques", filename="gie_risks.json.gz", date_str=date_str
        )
        upload_json(bucket_name, gie_key, gie_records)

        # Medallion Silver: Validate and upload as staging Parquet
        if gie_records:
            valid_gie = validate_records(gie_records, GieRiskRecord, pipeline_name="georisques_gie_risks")
            upload_parquet("datatouille", "landing/stg_georisques_gie_risks.parquet", valid_gie)

        logger.info("Mapped %d GIE risk profiles", len(gie_records))
    ...
    def load_to_postgres(gie_result: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error("Failed to load GIE data: %s", e)
            raise
        ...
        with pg_connection() as conn:
            with conn.cursor() as cur:
                ...
                insert_gie_risks(cur, gie_records)
                insert_risk_details(cur, risk_details)

        logger.info(
            "Loaded %d commune profiles, %d GIE risks, %d risk details to PostgreSQL",
            len(commune_profiles),
            len(gie_records),
            len(risk_details),
        )
        ...
        return {
            ...
            "gie_risks_loaded": len(gie_records),
    ...
    gie = transform_gie(transformed)
    postgres = load_to_postgres(gie)

    bucket >> raw >> transformed >> gie >> postgres
