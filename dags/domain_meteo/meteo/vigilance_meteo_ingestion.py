...
Extracts weather alerts from Meteo France API and maps them to Store locations.
...
from utils.s3 import download_json, ensure_bucket, get_partitioned_key, get_s3_client, upload_json
...
@dag(
    ...
    description="Vigilance Meteo France warnings mapped to Store stores",
...
def vigilance_meteo_ingestion():
    ...
    def transform_gie(alerts_result: dict[str, Any], **context) -> dict[str, Any]:
        """Map warnings to Store / Store locations."""
        cfg = get_meteo_config()
        bucket_name = cfg["vigilance_bucket_name"]
        ...
        # Map alerts
        gie_records = map_alerts_to_stores(alerts, store_mapping, date_str, extracted_at)

        # Store Store alerts
        gie_landing_key = get_partitioned_key(
            domain="meteo", source="vigilance", filename="store_alerts.json.gz", date_str=date_str
        )
        try:
            upload_json(bucket_name, gie_landing_key, gie_records)
        except (ClientError, EndpointConnectionError) as e:
            logger.error("Failed to upload Store alerts to S3 (key=%s): %s", gie_landing_key, e)
            raise

        logger.info("Mapped to %d Store alerts to s3://%s/%s", len(gie_records), bucket_name, gie_landing_key)
        return {
            "bucket": bucket_name,
    ...
    def load_to_postgres(gie_result: dict[str, Any]) -> dict[str, Any]:
        """Load alerts and Store alerts to PostgreSQL tables."""
        cfg = get_meteo_config()
        bucket_name = cfg["vigilance_bucket_name"]
        ...
        # Load Store alerts data
        try:
            store_alerts_raw = download_json(bucket_name, gie_result["gie_landing_key"])
            store_alerts: list[dict[str, Any]] = store_alerts_raw if isinstance(store_alerts_raw, list) else []
        except (ClientError, json.JSONDecodeError) as e:
            logger.error("Failed to load Store alerts from S3: %s", e)
            raise

        with pg_connection() as conn:
            with conn.cursor() as cur:
                insert_vigilance_alerts(cur, alerts)
                insert_store_alerts(cur, store_alerts)
                populate_vigilance_metadata(cur, f"{INPUTS_PATH}/dictionary.json")

        logger.info("Loaded %d alerts and %d Store alerts to PostgreSQL", len(alerts), len(store_alerts))
        return {
            "alerts_loaded": len(alerts),
            "store_alerts_loaded": len(store_alerts),
    ...
    store = transform_gie(alerts)
    postgres = load_to_postgres(store)

    bucket >> raw >> alerts >> store >> postgres
