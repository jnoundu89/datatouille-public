...
def _parse_listing_page(html: str) -> list[dict[str, str]]:
    ...
    for article in soup.find_all("article", class_="product_pod"):
        ...
        books.append({"url": url, "title": str(link.get("title") or "")})
    return books
...
def _parse_detail_page(
    ...
    try:
        ...
        star_rating = None
        if rating_elem:
            classes_val = rating_elem.get("class")
            classes = (
                classes_val if isinstance(classes_val, list) else [classes_val] if isinstance(classes_val, str) else []
            )
            for cls in classes:
                cls_str = str(cls)
                if cls_str.lower() in STAR_MAP:
                    star_rating = STAR_MAP[cls_str.lower()]
                    break
...
def _parse_quotes_page(html: str) -> list[dict[str, Any]]:
    ...
    for div in soup.find_all("div", class_="quote"):
        ...
        author_slug = None
        if author_link:
            href = str(author_link.get("href") or "")
...
def _parse_author_page(html: str, slug: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        return {
            ...
            "name": name[:200],
            "born_date": born_date,
            "born_location": born_location[:500] if born_location else "",
            "description": description,
        }
    except (AttributeError, ValueError, TypeError) as e:
