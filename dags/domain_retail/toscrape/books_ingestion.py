...
def toscrape_books_ingestion():
    ...
    def extract_details(listing_result: dict[str, Any], **context) -> str:
        ...
        with track_extraction("toscrape_books", DAG_ID, date_str, min_records=100) as metrics:
            ...
            from utils.validation import detect_anomalies

            df = pd.DataFrame(valid_records)
            df_analyzed = detect_anomalies(df, ["price", "stock_count"])

            # Quarantine anomalies vs Clean staging records
            anomalies_df = df_analyzed[df_analyzed["is_anomaly"]]
            clean_df = df_analyzed[~df_analyzed["is_anomaly"]]

            if not anomalies_df.empty:
                logger.warning(
                    "Detected %d anomalies in books ingestion! Quarantining to S3 anomalies bucket.", len(anomalies_df)
                )
                upload_parquet("datatouille", "landing/anomalies/stg_toscrape_books.parquet", anomalies_df)

            clean_df_final = clean_df.drop(columns=["is_anomaly", "anomaly_reasons"])
            upload_parquet("datatouille", "landing/stg_toscrape_books.parquet", clean_df_final)
