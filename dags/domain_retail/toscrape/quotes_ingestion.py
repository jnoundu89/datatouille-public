...
from utils.validation import AuthorRecord, QuoteRecord, validate_records
...
def toscrape_quotes_ingestion():
    ...
    def extract_quotes(bucket_result: dict, **context) -> dict[str, Any]:
        ...
        with track_extraction("toscrape_quotes", DAG_ID, date_str, min_records=50) as metrics:
            ...
            for page_name, raw_body in raw_pages.items():
                raw_html_key = get_partitioned_key(
                    domain="retail", source="toscrape_quotes", filename=f"{page_name}.html", date_str=date_str
                )
                s3.put_object(
                    Bucket=BUCKET,
                    Key=raw_html_key,
            ...
            landing_key = get_partitioned_key(
                domain="retail", source="toscrape_quotes", filename="quotes.json.gz", date_str=date_str
            )
            ...
            valid_quotes = validate_records(unique_quotes, QuoteRecord, pipeline_name="toscrape_quotes")
            upload_parquet("datatouille", "landing/stg_toscrape_quotes.parquet", valid_quotes)
    ...
    def extract_authors(quotes_result: dict[str, Any], **context) -> str:
        ...
        landing_key = get_partitioned_key(
            domain="retail", source="toscrape_quotes", filename="authors.json.gz", date_str=date_str
        )
        upload_json(BUCKET, landing_key, authors)
        logger.info("Stored %d authors", len(authors))

        # Medallion Silver: Validate and upload as staging Parquet
        valid_authors = validate_records(authors, AuthorRecord, pipeline_name="toscrape_authors")
        upload_parquet("datatouille", "landing/stg_toscrape_authors.parquet", valid_authors)
